...
int _configure_by_ubx(void)
{
    ...
    for (i = 0; i < sizeof(baudrates) / sizeof(baudrates[0]); i++) {
        ...
        if (_set_baudrate(serial_device, baudrate) == RT_EOK) {
        ...
        if (UBX_TX_CFG_PRT_BAUDRATE != baudrate) {
            if (_set_baudrate(serial_device, UBX_TX_CFG_PRT_BAUDRATE) == RT_EOK) {
    ...
    // if (_wait_for_ack(UBX_MSG_CFG_MSG, UBX_CONFIG_TIMEOUT) < 0) {
    //     DRV_DBG("UBX_MSG_NAV_DOP configure fail!\n");
    //     return 1;
    // }
    ...
    // if (_wait_for_ack(UBX_MSG_CFG_MSG, UBX_CONFIG_TIMEOUT) < 0) {
    //     DRV_DBG("UBX_MSG_NAV_SVINFO configure fail!\n");
    //     return 1;
    // }
    ...
    // if (_wait_for_ack(UBX_MSG_CFG_MSG, UBX_CONFIG_TIMEOUT) < 0) {
    //     DRV_DBG("UBX_MSG_MON_HW configure fail!\n");
    //     return 1;
    // }
...
rt_size_t gps_read(rt_device_t dev, rt_off_t pos, void* buffer, rt_size_t size)
{
    if (pos == RD_ONLY_POSLLH) {
        ...
        *(uint8_t*)buffer = (_got_posllh && _got_velned) ? 1 : 0;
    } else {
        return 0;
    }
...
rt_err_t drv_gps_init(char* serial_device_name)
{
    ...
    if (serial_device == RT_NULL) {
        DRV_DBG("gps can not find serial device:%s\n", serial_device_name);
        return RT_EEMPTY;
    }

    /* register to device manager */
    return rt_device_register(&gps_device, "gps", RT_DEVICE_FLAG_RDWR);
}

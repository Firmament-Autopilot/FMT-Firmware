...
static rt_err_t gyro_set_dlpf_filter(uint32_t frequency_hz)
{
    ...
    CHECK_RETURN(__modify_reg(imu_spi_dev, MPUREG_CONFIG, reg_val));
...
static rt_err_t gyro_set_range(uint32_t max_dps)
{
    ...
    CHECK_RETURN(__modify_reg(imu_spi_dev, GYRO_CONFIG, reg_val));
...
static rt_err_t accel_set_dlpf_filter(uint32_t frequency_hz)
{
    ...
    CHECK_RETURN(__modify_reg(imu_spi_dev, ACCEL_CONFIG_2, reg_val));
...
static rt_err_t accel_set_range(uint32_t max_g)
{
    ...
    CHECK_RETURN(__modify_reg(imu_spi_dev, ACCEL_CONFIG, reg_val));
...
static rt_err_t imu_init(void)
{
    ...
    CHECK_RETURN(__write_checked_reg(imu_spi_dev, MPUREG_CONFIG, 0x00));                             /* Gyro 8K rate, 250Hz BW */
    CHECK_RETURN(__modify_reg(imu_spi_dev, GYRO_CONFIG, REG_VAL(BIT(3) | BIT(4), BIT(0) | BIT(1)))); /* 2000dps, FCHOICE_B[0,0] */
    CHECK_RETURN(__modify_reg(imu_spi_dev, ACCEL_CONFIG, REG_VAL(BIT(3) | BIT(4), 0)));              /* 16g */
    CHECK_RETURN(__modify_reg(imu_spi_dev, ACCEL_CONFIG_2, REG_VAL(0, 0x0F)));                       /* Accel 1K rate, 218Hz BW */
    CHECK_RETURN(__write_checked_reg(imu_spi_dev, MPU_FIFO_EN_REG, 0x00));                           /* disable fifo */
    CHECK_RETURN(__write_checked_reg(imu_spi_dev, MPU_INT_EN_REG, 0x00));                            /* disable interrupts */
    CHECK_RETURN(__modify_reg(imu_spi_dev, USER_CONTROL, REG_VAL(BIT(4), BIT(6))));                  /* SPI only and disable fifo */
...
rt_err_t drv_icm20689_init(void)
{
    ...
    CHECK_RETURN(imu_init());

    /* register gyro hal device */
    CHECK_RETURN(hal_gyro_register(&gyro_dev, "gyro0", RT_DEVICE_FLAG_RDWR, RT_NULL));

    /* register accel hal device */
    CHECK_RETURN(hal_accel_register(&accel_dev, "accel0", RT_DEVICE_FLAG_RDWR, RT_NULL));

...
static void __handle_device_msg(void* msg)
{
    int idx;
...
static rt_device_t find_device(int idx)
{
            ...
            if (MATCH(key, "baudrate")) {
                ...
                if (toml_bool_in(curtab, key, &bval) == 0) {
                    ...
                    if (config->auto_switch) {
                        ...
                        if (device_mq_register(DEVICE_LIST[idx].name, __handle_device_msg) != FMT_EOK) {

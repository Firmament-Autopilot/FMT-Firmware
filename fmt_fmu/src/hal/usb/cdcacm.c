...
void cdc_connected_status_change(uint8_t connected)
{
    ...
    if (connected) {
        ...
        status = DEVICE_STAUTS_DISCONNECT;
    }
    devmq_notify(&usb_device, &status);
}
...
fmt_err usb_cdc_init(void)
{
    ...
    res = rt_device_register(&usb_device, USB_DEVICE_NAME, RT_DEVICE_FLAG_RDWR | RT_DEVICE_FLAG_STANDALONE | RT_DEVICE_FLAG_DMA_RX | RT_DEVICE_FLAG_DMA_TX);

    if (devmq_create(&usb_device, sizeof(device_status), 5) != FMT_EOK) {
        console_printf("fail to create usb devmq\n");
        return FMT_ERROR;
    }

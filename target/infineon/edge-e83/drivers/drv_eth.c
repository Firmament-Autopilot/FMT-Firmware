...
{
    Cy_GPIO_Pin_Init(GPIO_PRT11, 2U, &CYBSP_ETH_TXD_0_config);
    Cy_GPIO_Pin_Init(GPIO_PRT11, 3U, &CYBSP_ETH_TXD_1_config);

    Cy_GPIO_Pin_Init(GPIO_PRT11, 4U, &CYBSP_ETH_TX_CTL_config);

    Cy_GPIO_Pin_Init(GPIO_PRT11, 7U, &CYBSP_ETH_REF_CLK_config);

    Cy_GPIO_Pin_Init(GPIO_PRT10, 6U, &CYBSP_ETH_RXD_0_config);
    Cy_GPIO_Pin_Init(GPIO_PRT10, 7U, &CYBSP_ETH_RXD_1_config);

    Cy_GPIO_Pin_Init(GPIO_PRT10, 5U, &CYBSP_ETH_RX_CTL_config);

    Cy_GPIO_Pin_Init(GPIO_PRT13, 1U, &CYBSP_ETH_MDC_config);
    Cy_GPIO_Pin_Init(GPIO_PRT13, 6U, &CYBSP_ETH_MDIO_config);
}
...
#define PHY_REG_ANER            0x06U
#define PHY_REG_SPECIAL         0x1FU

#define PHY_BMCR_SPEED_100      (1U << 13)
#define PHY_BMCR_FULL_DUPLEX    (1U << 8)

#define PHY_SPECIAL_AUTODONE    (1U << 12)
#define PHY_SPECIAL_SPEED_MASK  (0x001CU)
#define PHY_SPECIAL_SPEED_10HD  (0x0004U)
#define PHY_SPECIAL_SPEED_10FD  (0x0014U)
#define PHY_SPECIAL_SPEED_100HD (0x0008U)
#define PHY_SPECIAL_SPEED_100FD (0x0018U)
...
static rt_err_t psoc_eth_hw_init(void)
{
    ...
    psoc_eth_mac_cfg.u8dmaCfgFlags = CY_ETHIF_CFG_DMA_FRCE_TX_BRST;
    ...
    psoc_eth_mac_cfg.u8enRxBadPreamble = 1U;

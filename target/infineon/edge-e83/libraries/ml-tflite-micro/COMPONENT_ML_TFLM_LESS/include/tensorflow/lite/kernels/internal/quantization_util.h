...
// IFX PATCH: Work-around for compiler/simulator bug on ETISS simulator
#if ((__GNUC__ == 12) && (__GNUC_MINOR__ ==1) && defined(ETISS_RISCV))
#include <type_traits>
#endif
...
IntOut SafeCast(FloatIn x) {
  ...
  // Handle infinities.
  // IFX PATCH armclang 6.18 has broken isinf.
#if __ARMCC_VERSION/10000==618
if (x == std::numeric_limits<FloatIn>::infinity() ) {
...
if (x == -std::numeric_limits<FloatIn>::infinity()) {
  ...
  // by the fall-through below.
   ...
   if (exp <= std::numeric_limits<IntOut>::digits) {
     // IFX PATCH: Workaround compiler/simulator bug...
#if ((__GNUC__ == 12) && (__GNUC_MINOR__ ==1) && defined(ETISS_RISCV))
    if (std::is_same<IntOut,uint32_t>::value && std::is_same<FloatIn,double>::value ) {
      auto recover = static_cast<uint64_t>(x);
      return recover > std::numeric_limits<IntOut>::max() 
            ? std::numeric_limits<IntOut>::max()
            : static_cast<IntOut>(recover);
    }
#endif
    return x;
  }

/***********************************************************************************************//**
 * \file mtb_serial_memory_prog.c
 *
 * \brief
 * Provides variables necessary to inform programming tools how to program the
 * attached serial memory. The variables used here must be placed at
 ...
 * be written at the same time as internal memory. This support can be enabled by defining
 * MTB_SERIAL_MEMORY_ENABLE_XIP_PROGRAM while building the application. With this define in
 * place, code will be generated in the .cy_sflash_user_data & .cy_toc_part2 sections. These
 * sections must be provided by the linker script for the application. One the application
 * has been built, these locations can be read by programming tools (eg: Cypress Programmer,
 * OpenOCD, pyOCD) to know that there is a memory device attached and how to program it.
...
#if defined(MTB_SERIAL_MEMORY_ENABLE_XIP_PROGRAM)
...
// (if any). This must consume a full row of memory memory row. The last entry is a checksum of the

...
USB_NOCACHE_RAM_SECTION struct dwc2_udc {
    ...
    volatile uint16_t last_sof_fn;
    volatile uint8_t disconnect_cnt;
...
static void dwc2_update_activity(uint8_t busid)
{
    g_dwc2_udc[busid].last_activity = rt_tick_get() * (1000 / RT_TICK_PER_SECOND);
}

void usbd_poll_connect_status(uint8_t busid)
{
    #define USB_OTG_DSTS_FNSOF_POS      8
    #define USB_OTG_DSTS_FNSOF_MASK     (0x3FFFUL << USB_OTG_DSTS_FNSOF_POS)
    #define USB_DISCONNECT_DEBOUNCE      3

    if (!g_dwc2_udc[busid].is_connected) {
        return;
    }

    uint16_t current_fn = (uint16_t)((USB_OTG_DEV->DSTS & USB_OTG_DSTS_FNSOF_MASK) >> USB_OTG_DSTS_FNSOF_POS);
    uint16_t last_fn = g_dwc2_udc[busid].last_sof_fn;

    if (current_fn != last_fn) {
        /* Frame number changed, SOF is alive */
        g_dwc2_udc[busid].last_sof_fn = current_fn;
        g_dwc2_udc[busid].disconnect_cnt = 0;
        return;
    }

    /* Frame number unchanged */
    g_dwc2_udc[busid].disconnect_cnt++;

    if (g_dwc2_udc[busid].disconnect_cnt >= USB_DISCONNECT_DEBOUNCE) {
        g_dwc2_udc[busid].is_connected = false;
        g_dwc2_udc[busid].disconnect_cnt = 0;
...
process_setup:
        ...
        if (gint_status & USB_OTG_GINTSTS_ENUMDNE) {
            ...
            /* Enumeration done - USB connected */
            if (!g_dwc2_udc[busid].is_connected) {
                g_dwc2_udc[busid].is_connected = true;
                dwc2_update_activity(busid);  /* Update activity timestamp */
                usbd_event_connect_handler(busid);
            } else {
                dwc2_update_activity(busid);  /* Still connected, update activity */
            }

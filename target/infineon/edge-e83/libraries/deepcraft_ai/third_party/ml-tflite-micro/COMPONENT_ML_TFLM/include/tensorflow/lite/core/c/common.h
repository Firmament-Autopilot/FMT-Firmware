...
typedef enum TfLiteQuantizationType {
  ...
  kTfLitePackedAffineQuantization = 3,
...
// IFX packed sub 8-bit quantized/sparse constants...

typedef struct TfLiteCustomSub8BitPackingDetails
{
  uint8_t    bits_per_item;  
  uint8_t    container_bits;  // gr_param in RG compression
  ...
  // For compression mode==CHANNELWISE_DENSE== the number
  // of channelwise filters in parallel
  // i.e. channels per "stripe"
  uint8_t     packed_minor_dims; // gr_mode in RG compression
  uint8_t     compression_type;

  union {
    uint32_t    rg_byte_offset_to_remainders;
    // Values from 1 to 255 interpreted as channel wise slicing in conv kernels.
    uint8_t     channel_wise_slicing_hint;
  } aux;
...
typedef struct TfLiteContext {
  ...
  void (*NotifyNodeIndex)(const struct TfLiteContext* context, size_t);

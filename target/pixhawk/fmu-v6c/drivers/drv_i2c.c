...
static void i2c1_hw_init(void)
{
    ...
    */
    ...
    I2C_InitStruct.Timing = 0x00701F6B; // 400KHz at 100MHz (approx) - Need to tune if needed, copying default safe value or one from CubeOrange if similar clock. 
                                      // Note: H743 I2C timing is clock dependent. Assuming typical.
                                      // Reference 0x00707CBB might be 100khz. 
                                      // Let's stick to CubeOrange value if clock is similar. 
                                      // Or use 0x6000030D from original file if valid.
    I2C_InitStruct.Timing = 0x00B03FDB; // 100kHz
    
    // For 400kHz from CubeMX reference usually looks different.
    // I will look at stm32h7xx_hal_msp.c to see if there is timing init? 
    // No, msp only does GPIO.
    // I will use a safe generic timing 0x20303E5D (Standard 100k-400k range) or keep existing if it worked for CubeOrange.
    I2C_InitStruct.Timing = 0x00701F6B; // 400kHz @ 64MHz?
...
static void i2c2_hw_init(void)
{
    ...
    */
    ...
    I2C_InitStruct.PeripheralMode = LL_I2C_MODE_I2C;
    I2C_InitStruct.Timing = 0x00701F6B;
    I2C_InitStruct.AnalogFilter = LL_I2C_ANALOGFILTER_ENABLE;
    I2C_InitStruct.DigitalFilter = 0;
    I2C_InitStruct.OwnAddress1 = 0;
    I2C_InitStruct.TypeAcknowledge = LL_I2C_ACK;
...
static rt_size_t i2c_master_transfer(
    struct rt_i2c_bus* bus, rt_uint16_t slave_addr, struct rt_i2c_msg msgs[], rt_uint32_t num)
{
...
_stop:
    ...
    if (!LL_I2C_IsActiveFlag_NACK(stm32_i2c->I2C)) {
        ...
        LL_I2C_HandleTransfer(
            stm32_i2c->I2C, slave_addr, LL_I2C_ADDRESSING_MODE_7BIT, 0, LL_I2C_MODE_SOFTEND, LL_I2C_GENERATE_STOP);
    }

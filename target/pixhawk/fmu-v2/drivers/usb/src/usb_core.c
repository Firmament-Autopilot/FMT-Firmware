...
USB_OTG_STS USB_OTG_HC_StartXfer(USB_OTG_CORE_HANDLE* pdev, uint8_t hc_num)
{
    ...
    if (pdev->host.hc[hc_num].xfer_len > 0) {
        num_packets = (pdev->host.hc[hc_num].xfer_len + pdev->host.hc[hc_num].max_packet - 1) / pdev->host.hc[hc_num].max_packet;

        if (num_packets > max_hc_pkt_count) {
            num_packets = max_hc_pkt_count;
            pdev->host.hc[hc_num].xfer_len = num_packets * pdev->host.hc[hc_num].max_packet;
        }
    } else {
        num_packets = 1;
    }

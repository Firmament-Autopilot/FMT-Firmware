...
static const struct dfs_mount_tbl mnt_table[] = {
    { "sd0", "/", "elm", 0, NULL }, { NULL } /* NULL indicate the end */
};
...
static void bsp_show_information(void)
{
    ...
    for (uint32_t i = 0; i < get_task_num(); i++) {
        ...
        banner_item(buffer,
            get_task_status(task_tab[i].name) == TASK_OK ? "OK" : "Fail",
            '.', ITEM_LENGTH);
    }
}
...
void SystemClock_Config(void)
{
    ...
    /* Intermediate AHB prescaler 2 when target frequency clock is higher than 80
     * MHz */
...
/* this function will be called before rtos start, which is not in the thread
 * context */
 ...
 */
void rt_hw_board_init() { bsp_early_initialize(); }

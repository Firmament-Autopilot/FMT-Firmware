...
static rt_err_t adc_hw_init(void)
{
    ...
    /* PC0 set to ADC123-IN10
       PC1 set to ADC123-IN11
       PC4 set to ADC12-IN14 */
    (void)GPIO_StructInit(&stcGpioInit);
    stcGpioInit.u16PinAttr = PIN_ATTR_ANALOG;
    (void)GPIO_Init(GPIO_PORT_C, GPIO_PIN_00 | GPIO_PIN_01 | GPIO_PIN_04, &stcGpioInit);

    /* 5. Set sampling time if needed. */
    ADC_SetSampleTime(CM_ADC1, ADC_CH10, 0x40U);
    ADC_SetSampleTime(CM_ADC1, ADC_CH11, 0x40U);
    ADC_SetSampleTime(CM_ADC1, ADC_CH14, 0x40U);
...
static rt_err_t enable(adc_dev_t adc_dev, uint8_t enable)
{
    if (enable == ADC_CMD_ENABLE) {
        ...
        ADC_ChCmd(CM_ADC1, ADC_SEQ_A, ADC_CH10, DISABLE);
        ADC_ChCmd(CM_ADC1, ADC_SEQ_A, ADC_CH11, DISABLE);
        ADC_ChCmd(CM_ADC1, ADC_SEQ_A, ADC_CH14, DISABLE);
    } else {
        return RT_EINVAL;
    }

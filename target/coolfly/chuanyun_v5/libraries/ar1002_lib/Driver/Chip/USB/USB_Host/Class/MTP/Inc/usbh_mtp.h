...
typedef struct _MTP_Process
{
  ...
  volatile MTP_StateTypeDef      state;
  MTP_EventHandleTypedef events;
  PTP_HandleTypeDef     ptp;
  uint32_t              current_storage_unit;
  uint32_t              is_ready;  
  uint32_t              mtp_send_timeout;
  uint32_t              mtp_send_timeout_times;
  volatile uint32_t     mtp_send_total_size;
  uint32_t              mtp_send_size;
  uint8_t               *mtp_send_buffer;
}
MTP_HandleTypeDef;

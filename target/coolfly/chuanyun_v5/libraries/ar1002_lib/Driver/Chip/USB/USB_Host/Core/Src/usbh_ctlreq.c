...
uint8_t usb_Ctrl_state;
uint8_t usb_URB_Status;
uint8_t usb_IDLE_count;
...
static USBH_StatusTypeDef USBH_HandleControl (USBH_HandleTypeDef *phost)
{
  ...
  usb_Ctrl_state = phost->Control.state;
  ...
  case CTRL_SETUP_WAIT:
    ...
    /*else if(URB_Status == USBH_URB_IDLE)
    {
      if(usb_IDLE_count++ > 3)
      {
        phost->Control.state = CTRL_ERROR;//CTRL_SETUP

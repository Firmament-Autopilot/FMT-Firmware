...
typedef struct _USBH_HandleTypeDef
{
  ...
  void                 (* isocURBDone )(struct _USBH_HandleTypeDef *pHandle);
#if (USBH_USE_OS == 1)
  ...
  uint32_t              disconn_tick;
  uint8_t               u8_bigEndianFlag;
  uint8_t               ClassRequestState;
  uint8_t               timeout_count;
  void                  (* recvCallback[15])(struct _USBH_HandleTypeDef * pHandle);
} USBH_HandleTypeDef;

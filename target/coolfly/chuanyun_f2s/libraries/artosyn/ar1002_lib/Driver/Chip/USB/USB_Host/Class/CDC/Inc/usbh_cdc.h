...
/**
  * @}
  */ 
#define CDC_SEND_BUFFER_MAX_SIZE                                16
...
typedef enum
{
  CDC_IDLE              = 0,
  CDC_SEND_DATA         = 1,
  CDC_SEND_DATA_WAIT    = 2,
  CDC_RECEIVE_DATA      = 3,
  CDC_RECEIVE_DATA_WAIT = 4,
}
...
typedef struct
{
  uint8_t              buff[8]  __attribute__ ((aligned (4)));             
  uint8_t              NotifPipe; 
  uint8_t              NotifEp;
  uint16_t             NotifEpSize;
  uint8_t              NotifInterval;
}
CDC_CommItfTypedef ;
...
#define NETCARD_SEND_BUFFER_EMPTY   0
#define NETCARD_SEND_BUFFER_WRITING 1
#define NETCARD_SEND_BUFFER_VALID   2
#define NETCARD_SEND_BUFFER_SENDING 3

typedef struct
{
    volatile    uint32_t data_size;
    volatile    uint32_t zero_padding;
    uint32_t    data[400];
    volatile    uint32_t wr_pos;
    volatile    uint32_t total_size;
    volatile    uint32_t data_valid;
} NETCARD_SEND_BUFFER_STRU;


typedef void(*CDC_Receive_Handler)(void *, uint32_t);
...
typedef struct _CDC_Process
{
  ...
  uint8_t                          *pTxData;
  uint8_t                          *pRxData;
  volatile uint32_t                 TxDataLength;
  volatile uint32_t                 RxDataLength;
  CDC_InterfaceDesc_Typedef         CDC_Desc;
  CDC_StateTypeDef                  state;
  volatile CDC_DataStateTypeDef     data_tx_state;
  volatile CDC_DataStateTypeDef     data_rx_state;
  volatile CDC_DataStateTypeDef     data_state;
  uint8_t                           Rx_Poll;
  void                             *pNetDevice;
  uint8_t                           enetaddr[6];
  CDC_Receive_Handler               HandleCdcRecvCallBack;
  uint8_t                           netcard_mode;               // 0: normal   1: promiscuous mode
  volatile uint8_t                  netcard_busy;
  volatile uint8_t                  net_line_connect;           // 0: disconnected   1: connected
}
CDC_HandleTypeDef;
...
void CDC_TransmitCallback(USBH_HandleTypeDef *phost);
//static void CDC_ReceiveInterruptCallback(USBH_HandleTypeDef *phost);
...
extern uint8_t                     g_cdcPortId;
extern volatile uint8_t            g_netcardSendBufferWrIndex;
extern volatile uint8_t            g_netcardSendBufferRdIndex;
extern NETCARD_SEND_BUFFER_STRU    g_netcardSendBuffer[CDC_SEND_BUFFER_MAX_SIZE];
extern uint8_t                     g_IPCameraMACaddr[6];
extern volatile uint32_t           g_setIPCameraMACaddr;

...
{
    IRQn_Type vct;
    uint32_t divider = SPI_CalcClkDiv(en_id, st_settings->clk_hz);
    ...
    Reg_Write32((SPI_BaseList[en_id] + SPI_BAUDR), divider); // LSB of BAUDR reg is always 0
    ...
    if (u8_spiIsr & SPI_ISR_TXEIS) // TX INT
    {
        ...
        while ((s_st_spiIntData[u8_spiCh].txAlrLen) < (s_st_spiIntData[u8_spiCh].txLen)) {
            if (0 == s_st_spiIntData[u8_spiCh].dfl) {
                ...
                Reg_Write32((SPI_BaseList[u8_spiCh] + SPI_DR),
                            (uint16_t)((s_st_spiIntData[u8_spiCh].txBuf[s_st_spiIntData[u8_spiCh].txAlrLen] << 8) | (s_st_spiIntData[u8_spiCh].txBuf[s_st_spiIntData[u8_spiCh].txAlrLen + 1]))); //write data
...
static uint32_t SPI_GetInputClockByIndex(ENUM_SPI_COMPONENT en_id)
{
    ...
    case SPI_5:
    case SPI_6:
        PLLCTRL_GetCoreClk(&u16_pllClk_MHz, ENUM_CPU0_ID);
        u16_pllClk_MHz = u16_pllClk_MHz >> 1; // clk div2,  e.g. 200MHz cpu --> 100MHz PLL
        break;
    case SPI_7:
        PLLCTRL_GetCoreClk(&u16_pllClk_MHz, ENUM_CPU2_ID);
        break;
    default:
        break;
    }

    return (uint32_t)u16_pllClk_MHz;
}

static uint32_t SPI_CalcClkDiv(ENUM_SPI_COMPONENT en_id, uint32_t u32_rate_hz)
{
    uint32_t u32_clk_MHz = SPI_GetInputClockByIndex(en_id);
    uint32_t u32_div;

    if (u32_rate_hz == 0)
        u32_rate_hz = 10 * 1000000;

    u32_div = u32_clk_MHz * 1000000 / u32_rate_hz;

...
USBH_StatusTypeDef USBH_UVC_InterfaceDeInit (USBH_HandleTypeDef *phost)
{
    ...
    if (g_UVCVideoBuffer != NULL)
    {
        ...
        {
            free_safe(g_UVCVideoBuffer->u8_frameBasedRecvBuff);
                          ...
                          UVC_SupportedFormatsDef uvc_format)
{
    ...
    if (g_UVCVideoBuffer == NULL)
    {
        g_UVCVideoBuffer = (USBH_UVCFrameBufferTypeDef *)malloc_safe(sizeof(USBH_UVCFrameBufferTypeDef));
    ...
    else
    {
        if (NULL == g_UVCVideoBuffer->u8_frameBasedRecvBuff)
        {
            g_UVCVideoBuffer->u8_frameBasedRecvBuff = (uint8_t *)malloc_safe(UVC_FRAME_BASED_RECV_SIZE);

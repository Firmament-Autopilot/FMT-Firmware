...
static rt_err_t gyro_control(gyro_dev_t gyro, int cmd, void* arg) { return RT_EOK; }
...
static rt_err_t accel_control(accel_dev_t accel, int cmd, void* arg) { return RT_EOK; }
...
static struct gyro_device gyro_dev = { .ops = &_gyro_ops, .config = GYRO_CONFIGURE, .bus_type = GYRO_SPI_BUS_TYPE };

static struct accel_device accel_dev = { .ops = &_accel_ops, .config = ACCEL_CONFIGURE, .bus_type = GYRO_SPI_BUS_TYPE };

static struct mag_device mag_dev = { .ops = &__mag_ops, .config = AK09916_CONFIG_DEFAULT, .bus_type = MAG_SPI_BUS_TYPE };

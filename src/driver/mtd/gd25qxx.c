...
#define GD25QXX_CMD_PAGE_READ          0x03
...
#define GD25QXX_CMD_READ_STATUS2       0x35
#define GD25QXX_CMD_RELEASE_PWOER_DOWN 0xAB
...
static rt_err_t probe(mtd_dev_t mtd_dev)
{
    ...
    case 0x18:
        ...
        // TODO, 32-bit address
        printf("GD25QXX driver dn not support 32-bit address yet\n");
        return RT_ENOSYS;
        break;
    case 0x19:
        /* GD25Q512 */
...
{
    uint8_t status = 0;
    uint8_t cmd = GD25QXX_CMD_READ_STATUS1;
    volatile uint32_t timeout_cnt = 100000UL;

    while (timeout_cnt-- != 0) {
        // status = rt_spi_sendrecv8((struct rt_spi_device*)mtd_spi_dev, GD25QXX_CMD_READ_STATUS1);
...
static rt_err_t release_powerdown(void)
{
    ...
    uint8_t cmd = GD25QXX_CMD_RELEASE_PWOER_DOWN;
...
{
    struct rt_spi_message message1;
    uint8_t cmd = GD25QXX_CMD_WRITE_DISABLE;
...
{
    struct rt_spi_message message1;
    uint8_t cmd = GD25QXX_CMD_WRITE_ENABLE;
...
static rt_err_t page_read(uint32_t addr, uint8_t* buffer, uint32_t len)
{
    ...
    LOAD_CMD(cmd, GD25QXX_CMD_PAGE_READ, addr);
...
static rt_err_t page_write(uint32_t addr, const uint8_t* buffer, uint32_t len)
{
    ...
    LOAD_CMD(cmd, GD25QXX_CMD_PAGE_PROGRAM, addr);
...
static rt_err_t sector_erase(uint32_t addr)
{
    uint8_t cmd[4];
    rt_size_t cnt;
    LOAD_CMD(cmd, GD25QXX_CMD_SECTOR_ERASE, addr);
...
static rt_err_t gd25qxx_read(mtd_dev_t mtd, rt_uint8_t* buffer, rt_uint32_t sector, rt_uint32_t count)
{
...
static rt_err_t gd25qxx_write(mtd_dev_t mtd, const rt_uint8_t* buffer, rt_uint32_t sector, rt_uint32_t count)
{
    uint32_t addr = sector * SECTOR_SIZE;
...
static rt_err_t gd25qxx_erase(mtd_dev_t mtd, rt_uint32_t sector, rt_uint32_t count)
{
    return sector_erase(sector * SECTOR_SIZE);
}

static rt_err_t gd25qxx_control(mtd_dev_t mtd, int cmd, void* arg)
{
    switch (cmd) {
    ...
    case RT_DEVICE_CTRL_BLK_ERASE: {
        ...
        RT_TRY(gd25qxx_erase(mtd, sectors[0], count));
    } break;
    default: {
...
const static struct mtd_ops dev_ops = {
    ...
    .write = gd25qxx_write,
    .erase = gd25qxx_erase,
    .control = gd25qxx_control
};

rt_err_t drv_gd25qxx_init(const char* spi_device_name, const char* mtd_device_name)
{
    static struct mtd_device gd25qxx_dev;
    ...
    /* probe the device */
    RT_TRY(probe(&gd25qxx_dev));
    /* power-on the device and disable write by default */
    RT_TRY(release_powerdown());
    RT_TRY(write_disable());

    gd25qxx_dev.ops = &dev_ops;
    return hal_mtd_register(&gd25qxx_dev, mtd_device_name, RT_DEVICE_FLAG_RDWR | RT_DEVICE_FLAG_STANDALONE, RT_NULL);
}
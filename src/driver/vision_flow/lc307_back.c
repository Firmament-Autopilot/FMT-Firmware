...
static int32_t lc307_cmd_AA_AB(void)
{
    ...
    buf[0] = 0xAA _writ_reg(dev, buf, 1);
    ...
    if ((recv_buf[0] != 0xAB) || (recv_buf[1] != 0x00) || (recv_buf[2] != 0xAB)) {
        DLOG_Error("=>>>>>>> lc307 0xAB RETURN ERROR!!! ");
        return -2;
    }
}
        ...
        if ((res == RT_EOK && (recv_set & EVENT_LC307_UPDATE)) || res == -RT_ETIMEOUT) {
            while (rt_device_read(dev, 0, &c, 1) > 0) {
                if (parse_package(c) == true) {
                    ...
                    //gyro low pass filter (delay) for fitting flow data()
                    LPF_1_(3.0f, dT, imu_report.gyr_B_radDs[0], gyro_lpf_x);
                    LPF_1_(3.0f, dT, imu_report.gyr_B_radDs[1], gyro_lpf_y);

                    //rotation compensation	mm
                    float fx_gyro_fix = (fx / dT - constrain(gyro_lpf_y * 10000, -10000, 10000)) * 0.0001 * use_height;
                    float fy_gyro_fix = (fy / dT + constrain(gyro_lpf_x * 10000, -10000, 10000)) * 0.0001 * use_height;

                    Vector3f delta_b(fx_gyro_fix, fy_gyro_fix, 0);

                    _R_att = Dcm<float>(INS_Out.quat); // 获取旋转矩阵
                    ...
                    y(Y_flow_vx) = constrain(Y_flow_vx, -0.3f, 0.3f);
                    y(Y_flow_vy) = constrain(Y_flow_vy, -0.3f, 0.3f);

                    return OK;

                    optflow_report.vx_mPs = y(Y_flow_vx) / 0.01f;

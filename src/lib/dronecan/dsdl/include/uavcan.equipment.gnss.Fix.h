...
static inline void _uavcan_equipment_gnss_Fix_decode(const CanardRxTransfer* transfer, uint32_t* bit_ofs, struct uavcan_equipment_gnss_Fix* msg, bool tao);
void _uavcan_equipment_gnss_Fix_encode(uint8_t* buffer, uint32_t* bit_ofs, struct uavcan_equipment_gnss_Fix* msg, bool tao) {
    ...
    *bit_ofs += 16;
    *bit_ofs += 4;
    canardEncodeScalar(buffer, *bit_ofs, 4, &msg->position_covariance.len);
    *bit_ofs += 4;
    ...
    if (!tao) {
        canardEncodeScalar(buffer, *bit_ofs, 4, &msg->velocity_covariance.len);
        *bit_ofs += 4;
    }
...
void _uavcan_equipment_gnss_Fix_decode(const CanardRxTransfer* transfer, uint32_t* bit_ofs, struct uavcan_equipment_gnss_Fix* msg, bool tao) {
    (void)transfer;
    (void)bit_ofs;
    (void)msg;
    (void)tao;

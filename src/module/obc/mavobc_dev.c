...
static rt_device_t mavobc_dev = RT_NULL;
static struct rt_completion tx_cplt, rx_cplt;

static fmt_err_t (*mav_rx_indicate)(uint32_t size) = NULL;

static rt_err_t mavobc_dev_tx_done(rt_device_t dev, void* buffer)
{
    rt_completion_done(&tx_cplt);
    return RT_EOK;
}

static rt_err_t mavobc_dev_rx_ind(rt_device_t dev, rt_size_t size)
{
...
rt_size_t mavobc_dev_write(const void* buffer, uint32_t len, int32_t timeout)
{
    rt_size_t size;

    if (mavobc_dev == NULL) {
        /* mavobc device not initialized */
        return 0;
    }
    /* write data to device */
    size = rt_device_write(mavobc_dev, 0, buffer, len);
    if (size > 0) {
...
rt_size_t mavobc_dev_read(void* buffer, uint32_t len, int32_t timeout)
{
    rt_size_t cnt = 0;
    ...
    /* try to read data */
    cnt = rt_device_read(mavobc_dev, 0, buffer, len);
    ...
    if (timeout != 0) {
        ...
        while (cnt < len) {
            ...
            /* read rest data */
            cnt += rt_device_read(mavobc_dev, 0, (void*)((uint32_t)buffer + cnt), len - cnt);
        }
    }
...
void mavobc_dev_set_rx_indicate(fmt_err_t (*rx_ind)(uint32_t size))
{
    mav_rx_indicate = rx_ind;
}

fmt_err_t mavobc_set_device(const char* dev_name)
{
    rt_device_t new_dev;
    ...
    rt_device_set_tx_complete(new_dev, mavobc_dev_tx_done);
    rt_device_set_rx_indicate(new_dev, mavobc_dev_rx_ind);

    return FMT_EOK;
}

rt_device_t mavobc_get_device(void)
{
    return mavobc_dev;
}

fmt_err_t mavobc_dev_init(void)
{

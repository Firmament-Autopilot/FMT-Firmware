...
static uint8_t UnpackData(const uint8_t *data, size_t data_length) {
  if (data_length < g_nlt_nodeframe6.fixed_part_size ||
      data[0] != g_nlt_nodeframe6.frame_header ||
      data[1] != g_nlt_nodeframe6.function_mark)
    return 0;
  ...
  if (init_needed) {
    memset(g_nlt_nodeframe6.result.nodes, 0,
           sizeof(g_nlt_nodeframe6.result.nodes));
    init_needed = 0;
  }

  memcpy(&g_frame, data, g_nlt_nodeframe6.fixed_part_size);
  g_nlt_nodeframe6.result.role = g_frame.role;
  ...
  for (size_t i = 0, address = g_nlt_nodeframe6.fixed_part_size,
    ...
    TRY_MALLOC_NEW_NODE(g_nlt_nodeframe6.result.nodes[i], nlt_nodeframe6_node_t)
    nlt_nodeframe6_node_t *node = g_nlt_nodeframe6.result.nodes[i];
    node->role = begin[0];
...
nlt_nodeframe6_t g_nlt_nodeframe6 = {.fixed_part_size = 14,
                                     .frame_header = 0x55,
                                     .function_mark = 0x09,

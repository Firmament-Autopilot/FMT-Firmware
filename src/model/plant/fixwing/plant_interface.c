...
static param_t __param_list[] = {
    PARAM_DOUBLE(LAT_0, 0.65673),
    PARAM_DOUBLE(LON_0, -2.1361),
    PARAM_DOUBLE(ALT_0, 4.5),
};
...
static void publish_sensor_data(uint32_t timestamp)
{
    ...
    if (Plant_Y.Airspeed.timestamp != airspeed_timestamp) {
        airspeed_data_t airspeed_report;

        airspeed_report.timestamp_ms     = timestamp;
        airspeed_report.diff_pressure_pa = Plant_Y.Airspeed.diff_pressure;
        airspeed_report.temperature_deg  = Plant_Y.Airspeed.temperature;

        // publish sensor_gps data
        mcn_publish(MCN_HUB(sensor_airspeed), &airspeed_report);

        airspeed_timestamp = Plant_Y.Airspeed.timestamp;
    }
}

/*
 * File: FMS_data.c
 *
 * Code generated for Simulink model 'FMS'.
 *
 * Model version                  : 1.2061
 * Simulink Coder version         : 9.0 (R2018b) 24-May-2018
 * C/C++ source code generated on : Thu Apr  3 16:44:07 2025
 *
 * Target selection: ert.tlc
 * Embedded hardware selection: ARM Compatible->ARM Cortex
 * Code generation objectives: Unspecified
 * Validation result: Not run
 */

#include "FMS.h"
#include "FMS_private.h"

/* Invariant block signals (default storage) */
const ConstB_FMS_T FMS_ConstB = {
  1.9966471893352524,                  /* '<S151>/Sum' */
  0.0066943799901413165,               /* '<S151>/Multiply3' */
  0.99330562000985867,                 /* '<S151>/Sum4' */
  0.25F,                               /* '<S90>/Square' */
  4.903F,                              /* '<S90>/Multiply' */
  -19.612F,                            /* '<S90>/Gain4' */

  { 0.0F, 0.0F, 1.0F },                /* '<S120>/Vector Concatenate3' */

  { 0.0F, 0.0F, 1.0F },                /* '<S122>/Vector Concatenate3' */
  0.25F,                               /* '<S131>/Square' */
  4.903F,                              /* '<S131>/Multiply' */
  -19.612F,                            /* '<S131>/Gain4' */
  3U,                                  /* '<S33>/Data Type Conversion' */
  12U,                                 /* '<S33>/Data Type Conversion1' */
  1U,                                  /* '<S33>/Data Type Conversion2' */
  3U,                                  /* '<S36>/Data Type Conversion' */
  10U,                                 /* '<S36>/Data Type Conversion1' */
  5U,                                  /* '<S36>/Data Type Conversion2' */
  3U,                                  /* '<S37>/Data Type Conversion' */
  8U,                                  /* '<S37>/Data Type Conversion1' */
  3U,                                  /* '<S37>/Data Type Conversion2' */
  3U,                                  /* '<S86>/Data Type Conversion' */
  4U,                                  /* '<S86>/Data Type Conversion1' */
  5U,                                  /* '<S86>/Data Type Conversion2' */
  3U,                                  /* '<S84>/Data Type Conversion' */
  3U,                                  /* '<S84>/Data Type Conversion1' */
  6U,                                  /* '<S84>/Data Type Conversion2' */
  3U,                                  /* '<S125>/Data Type Conversion' */
  6U,                                  /* '<S125>/Data Type Conversion1' */
  5U,                                  /* '<S125>/Data Type Conversion2' */
  3U,                                  /* '<S126>/Data Type Conversion' */
  16U,                                 /* '<S126>/Data Type Conversion1' */
  5U,                                  /* '<S126>/Data Type Conversion2' */
  2U,                                  /* '<S30>/Data Type Conversion2' */
  2U,                                  /* '<S30>/Data Type Conversion1' */
  1U,                                  /* '<S29>/Data Type Conversion' */
  1U,                                  /* '<S29>/Data Type Conversion1' */

  /* Start of '<S27>/Unknown' */
  {
    1U,                                /* '<S35>/Data Type Conversion' */
    1U                                 /* '<S35>/Data Type Conversion1' */
  }
  ,

  /* End of '<S27>/Unknown' */

  /* Start of '<S31>/Unknown' */
  {
    1U,                                /* '<S38>/Data Type Conversion' */
    1U                                 /* '<S38>/Data Type Conversion1' */
  }
  ,

  /* End of '<S31>/Unknown' */

  /* Start of '<S32>/Unknown' */
  {
    1U,                                /* '<S85>/Data Type Conversion' */
    1U                                 /* '<S85>/Data Type Conversion1' */
  }
  ,

  /* End of '<S32>/Unknown' */

  /* Start of '<S34>/Unknown' */
  {
    1U,                                /* '<S127>/Data Type Conversion' */
    1U                                 /* '<S127>/Data Type Conversion1' */
  }
  /* End of '<S34>/Unknown' */
};

/*
 * File trailer for generated code.
 *
 * [EOF]
 */

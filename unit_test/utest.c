...
int utest_init(void)
{
    ...
    // ulog_i(DBG_TAG, "utest is initialize success.");
    // ulog_i(DBG_TAG, "total utest testcase num: (%d)", tc_num);
...
static void utest_tc_list(void)
{
    ...
    ulog_i(DBG_TAG, "Commands list : ");

    for (i = 0; i < tc_num; i++) {
        ulog_i(DBG_TAG, "[testcase name]:%s; [run timeout]:%d", tc_table[i].name, tc_table[i].run_timeout);
    }
}
...
static const char* file_basename(const char* file)
{
    ...
    if (!((end_ptr = strrchr(file, '\\')) != RT_NULL || (end_ptr = strrchr(file, '/')) != RT_NULL) || (rt_strlen(file) < 2)) {
        rst = (char*)file;
    } else {
        rst = (char*)(end_ptr + 1);
    }
...
static void utest_run(const char* utest_name)
{
    ...
    for (index = 0; index < tc_loop; index++) {
        i = 0;
        is_find = RT_FALSE;
        ulog_i(DBG_TAG, "[==========] [ utest    ] loop %d/%d", index + 1, tc_loop);
        ulog_i(DBG_TAG, "[==========] [ utest    ] started");
        while (i < tc_num) {
            if (utest_name) {
            ...
            ulog_i(DBG_TAG, "[----------] [ testcase ] (%s) started", tc_table[i].name);
            if (tc_table[i].init != RT_NULL) {
                if (tc_table[i].init() != RT_EOK) {
                    ulog_e(DBG_TAG, "[  FAILED  ] [ result   ] testcase (%s)", tc_table[i].name);
                    goto __tc_continue;
                }
            }
            ...
            if (tc_table[i].tc != RT_NULL) {
                ...
                if (local_utest.failed_num == 0) {
                    ulog_i(DBG_TAG, "[  PASSED  ] [ result   ] testcase (%s)", tc_table[i].name);
                } else {
                    ulog_e(DBG_TAG, "[  FAILED  ] [ result   ] testcase (%s)", tc_table[i].name);
                }
            } else {
        ...
        __tc_continue:
            ulog_i(DBG_TAG, "[----------] [ testcase ] (%s) finished", tc_table[i].name);

            i++;
        }

        if (i == tc_num && is_find == RT_FALSE && utest_name != RT_NULL) {
            ulog_i(DBG_TAG, "[==========] [ utest    ] Not find (%s)", utest_name);
            ulog_i(DBG_TAG, "[==========] [ utest    ] finished");
            break;
        }
...
static void utest_testcase_run(int argc, char** argv)
{
    ...
    if (argc == 1) {
        ...
        ulog_e(DBG_TAG, "[  error   ] at (%s:%d), in param error.", __func__, __LINE__);
        utest_help();
    }
}
...
void utest_assert(int value, const char* file, int line, const char* func, const char* msg)
{
    if (!(value)) {
        ...
        ulog_e(DBG_TAG, "[  ASSERT  ] [ unit     ] at (%s); func: (%s:%d); msg: (%s)", file_basename(file), func, line, msg);
    } else {
        ulog_d(DBG_TAG, "[    OK    ] [ unit     ] (%s:%d) is passed", func, line);
        local_utest.error = UTEST_PASSED;
